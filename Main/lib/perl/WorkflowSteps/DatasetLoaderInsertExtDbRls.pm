...
sub run {
    ...
    # otherwise insert this ext db rls
      ...
      $self->error("DatasetLoader $datasetName declares releaseDate=$releaseDate.  The value is not in yyyy-mm-dd format (eg 2012-03-27).") if ($releaseDate && $releaseDate !~ /^\d\d\d\d-\d\d-\d\d$/);
