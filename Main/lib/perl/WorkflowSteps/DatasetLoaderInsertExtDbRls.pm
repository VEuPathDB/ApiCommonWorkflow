...
sub run {
    ...
    # otherwise insert this ext db rls
      ...
      $self->error("DatasetLoader $datasetName declares releaseDate=$releaseDate.  The value is not in yy-mmm-dd format (eg 12-MAR-27).") if ($releaseDate && $releaseDate !~ /^\d\d-\D\D\D-\d\d$/);
