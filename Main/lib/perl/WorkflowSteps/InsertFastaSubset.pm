...
my $args = "--externalDatabaseName $extDbName --externalDatabaseVersion $extDbVer --sequenceFile $workflowDataDir/$fastaFile --sourceIdsFile  $workflowDataDir/$idsFile --regexSecondaryId 'gi\\|\\d+\\|\\w+\\|(\\w+\\.?\\w+)\\|' --regexDesc '>gi\\|\\d+\\|\\w+\\|\\w+\\.?\\w+\\|(\.+)' --regexSourceId '^>?(\\S+)\\s' --tableName DoTS::ExternalAASequence";
