...
sub run {
  ...
  if ($undo){
      ...
      my $sql = "delete from ApiDB.GENETABLE where source_id in (select distinct source_id from webready.GeneAttributes where organism='$organismFullName' and is_deprecated=$deprecated)";
